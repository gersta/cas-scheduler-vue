...
describe("Lecture details", () => {
  ...
  it("should display the lecture owner", () => {
    cy.get(".cas-lecture-owner").should("have.text", "Prof. Dr. Volker Schulz");
  });
...
describe("Missing lecture details", () => {
  it("should display hint message for missing details", () => {
    ...
    cy.get(".lecture-item")
      .filter(":visible") // skip elements hidden via v-if
      .first()
      .should("contain.text", "T3M32004")
      .click()

    cy.get(".cas-lecture-details")
      .should(
        "contain.text",
        "There is no entry for the lecture T3M32004 in the module handbook. Lecture details are not available."
      );
  });
});
