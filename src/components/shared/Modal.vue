          ...
          >
            ...
            >
              ...
              <div class="p-1 w-full flex flex-col justify-center items-center">
                <div><span class="cas-text-small">Start:</span> {{ getFormattedDate(index, "start") }}</div>
                <div><span class="cas-text-small">End:</span> {{ getFormattedDate(index, "end") }}</div>
              </div>
            </a>
          </button>
        </div>
        <div class="m-4 object-bottom" @click="$emit('closeModal')">
          <hr />
